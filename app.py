...
st.markdown("""
    ...
    .pnl-pill {
        ...
        margin-left: 8px; border: 1px solid rgba(255,255,255,0.1);
    }
...
with st.container():
    ...
    st.markdown("<h3 style='margin-top:0;'>💼 持仓成本看板</h3>", unsafe_allow_html=True)
    p_col1, p_col2, p_col3 = st.columns([1.5, 1.5, 1])
    with p_col1: new_ticker = st.text_input("股票代码", placeholder="BTDR").upper()
    with p_col2: new_cost = st.number_input("持有成本", min_value=0.0, step=0.01, format="%.2f")
    with p_col3:
        st.write("<div style='height:28px;'></div>", unsafe_allow_html=True)
        if st.button("更新资产", use_container_width=True):
            if new_ticker:
                st.session_state.portfolio[new_ticker] = new_cost
                st.toast(f"✅ {new_ticker} 成本已记录")
    ...
    with st.container():
        ...
        with col1:
            ...
            st.markdown(f"""
                <div class='apple-card'>
                    ...
                    <table class='f-table'>
                        <tr><td>现价</td><td class='f-val'>${item['curr_p']:.2f}</td></tr>
        ...
        with col2:
            fig = go.Figure(data=[go.Candlestick(x=item['df'].index, open=item['df']['Open'], high=item['df']['High'], low=item['df']['Low'], close=item['df']['Close'],
                increasing_line_color='#FF3B30', decreasing_line_color='#34C759', increasing_fillcolor='#FF3B30', decreasing_fillcolor='#34C759')])
            for t in item['trades']:
                is_buy = t['type'] == 'buy'
                fig.add_trace(go.Scatter(x=[t['date']], y=[t['price']], mode='markers+text', marker=dict(symbol="triangle-up" if is_buy else "triangle-down", size=10, color="#FF3B30" if is_buy else "#34C759"), text=["B" if is_buy else "S"], textposition="top center", showlegend=False))
            ...
            st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False}, key=f"v15_{item['ticker']}")
        st.divider()
