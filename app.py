...
for ticker in ticker_list:
    try:
        ...
        tags = []
        if "bitcoin" in summary or "crypto" in summary: tags.append("加密矿业")
        if "ai" in summary or "intelligence" in summary: tags.append("AI算力")
        if "defense" in summary: tags.append("国防科技")
        
        processed_data.append({
            "ticker": ticker, "profit": profit, "trades": trades, 
            "is_odd": (len(trades) % 2 != 0), "info": info, "df": test_df, "tags": tags
        })
    except: continue
        ...
        with col2:
            ...
            for t in item['trades']:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    text=["B" if is_buy else "S"], textposition="top center",
                    showlegend=False
                ))
            ...
            st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False}, key=f"v7_{item['ticker']}")
        st.divider()
